...
# 后续 Phase 0.66-0.73 每消掉一组硬编码，就把对应断言从 WILL_FAIL 债务
...
# 1. HitZoneMatcher 不应把 click pool 和 hit zone 写死绑定。
_check_absent(1 0.66
    "apps/desktop/src/pet/interaction/HitZoneMatcher.cpp"
    "click."
    "HitZoneMatcher must not know skin click pool ids"
)

# 2. 双击默认池应来自 manifest clickBehaviors.doubleClick。
_check_absent(2 0.66
    "apps/desktop/src/pet/interaction/InteractionPipeline.cpp"
    "doubleClick.random"
    "double click pool must be configured by manifest"
)

# 3. 拖拽晃动分支应由 GestureTracker + behavior rules 表达。
_check_absent(3 0.67
...
# 4. 睡眠菜单应读取 rest capability，而不是写死 Miles recipe。
...
# 5. 睡眠状态应读取 rest capability，而不是比较 action id。
...
# 7. 启动序列应由 runtime.started trigger 选择。
...
# 8. walk/run 播完后的续接动作应由 action.completed trigger 选择。
_check_absent(8 0.68
    "apps/desktop/src/pet/PetRuntime.cpp"
    "walk.finished"
...
# 9. movementDirection 到 facing 的映射应来自 manifest.movementFacingMap。
_check_absent(9 0.68
    "apps/desktop/src/pet/PetRuntime.cpp"
    "QStringLiteral(\"east\")"
...
# 11. 尺寸档位和语音语言不应硬编码在 PetRuntime。
_check_absent(11 0.69
    "apps/desktop/src/pet/PetRuntime.cpp"
    "\"mini\""
...
# 12. 表面层判断 idle loop 不应写死 Miles idle action。
_check_absent(12 0.69
    "apps/desktop/src/pet/surface/PetSurfaceWindow.cpp"
    "idle_stand"
    "surface idle loop detection must be manifest-driven"
)

# 13. CustomInteractionRegistry 不应停留在空壳。
_check_absent(13 0.70
