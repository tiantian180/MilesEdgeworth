...
- 皮肤包、Clip、Action、Recipe、AnimationPool、BehaviorRule 和 Custom Interaction 的详细层级模型，以 `docs/v2/设计方案/皮肤包播放行为设计.md` 为准。
...
当前 `ActionRequestKind` 已包含 `AnimationPool`、`Recipe`、`Action`、`ReturnToIdle`、`ToggleFacing`、`SpawnProp` 和 `PlaySound`。`interruptHint=afterCurrent` 已支持一个 `pendingRequest` 槽：当当前动作有自然完成边界时，新请求会等当前动作播完后执行；更复杂的多队列、优先级抢占和 phase exit 编排仍应在 `PlaybackController` / `RecipeRunner` 拆分时统一实现。
...
Clip 是最小可复用播放片段，只负责播放素材。运行时只播放完整 GIF（统一使用 QMovie），不做运行时帧段截取。

Action variant 通过 `clip` 字段引用播放素材，有两种值形式：
- **裸名**（`thinking.enter.right`）：引用 manifest 顶层 `clips` section 中的定义，对应构建脚本预切分到 `generated/clips/{name}.gif` 的完整 GIF
- **`file:` 路径**（`file:assets/body/idle/stand-right.gif`）：直接引用皮肤包内现有的完整 GIF，无需定义 clip，无需构建脚本切分

`clips` section 仅用于需要从原始 GIF 切分帧段的场景（例如多个 action phase 共享同一个源文件的不同帧段）。完整 GIF 直接用 `file:` 引用，不需要在 `clips` 中定义。
...
```json
{
  ...
  "actions": {
    "thinking": {
      ...
      "variants": {
        "right": { "clip": "file:assets/body/gestures/thinking-right.gif" },
        "left": { "clip": "file:assets/body/gestures/thinking-left.gif" }
      }
    }
  }
}
...
同一 GIF 标出局部循环段后，帧段定义写入 `clips`，action phase 通过 clip 名引用：
...
MotionController 详细设计见 `移动系统与工具调用设计.md` §5。以下是概要。

MotionController 由 PetRuntime 持有，提供 `moveTo(x, y, mode)`、`moveBy(dx, dy, mode)` 和 `stop()` 三个接口。坐标使用屏幕归一化小数（0.0~1.0），Qt 侧解析为实际像素；中心是 `(0.5, 0.5)`，不是 `(50, 50)`。

内部使用独立 QTimer（~16ms）驱动，与 GIF 帧率解耦。每 tick 计算方向向量、量化为 8 方向、按速度移动、接近目标后吸附。

速度参数由 manifest `motion` 节声明：
...
速度单位为像素/秒（scale=1 基准），实际速度 = `baseSpeed × petScale`。

大模型通过 OpenAI-compatible tool use 协议表达移动意图：
...
Go sidecar 解析 tool_calls，emit SSE 事件通知 Qt，Qt 执行移动后通过 HTTP 回传 tool result。详见 `移动系统与工具调用设计.md` §2–§4。

Miles 皮肤的 8 方向映射保持不变：

```text
east / west / northEast / northWest / southEast / southWest / north / south
```

方向变化时 PetRuntime 调用 `playLocomotion` 切换走/跑动画变体，并通过 `movementFacingMap` 更新朝向。

当前的随机走/跑（由 BehaviorTriggerEngine 触发、`consumeFrameMovementDelta()` 逐帧读取）保持不变，与 MotionController 的目标点移动是两套独立移动源。MotionController 执行目标点移动时暂停随机漫步。
...
Phase 2.5 引入 OpenAI-compatible tool use。模型可以在流式回复中输出 `tool_calls`，sidecar 暂停流式输出、通知 Qt 执行 tool、等待结果后继续生成。

移动是第一个通过 tool use 接入的能力。模型调用 `pet_motion` tool 请求桌宠移动，ChatController 收到 TOOL_CALL SSE 事件后，先等当前文字段吐完 + 动画 cleanFinish，再将请求转给 PetRuntime → MotionController 执行。移动完成后 ChatController 通过 HTTP 回传 tool result，sidecar 继续与 provider 对话。

移动和说话互斥：EXECUTING_TOOL 状态期间，输入框禁用，文字不流出，桌宠播放走/跑动画。PetRuntime 通过 `enterMovingState()` / `exitMovingState()` 管理移动状态（不使用 `setState("moving")`），设置 `m_motionLoopOverride` 让 walk/run 动画以 `loopMode="loop"` 持续循环。

sidecar 的 `chat.Message` 模型扩展支持 `tool_calls` 字段（assistant 角色）和 `tool` role message（回传 tool result），以支持 OpenAI-compatible 多轮 tool 调用。双层超时保障：Qt 侧 90 秒 motion timeout（主动 stop + POST result），sidecar 120 秒兜底超时。用户取消时通过 context cancel 清理。

完整 tool use 协议、SSE 事件序列、Message 模型扩展、超时/取消语义和 MotionController 架构详见 `移动系统与工具调用设计.md`。
