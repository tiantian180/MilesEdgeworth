...
**落盘位置**：内置皮肤位于 qrc / app bundle 内，不可写。因此 persona 文件的读写分两种情况：

| 皮肤类型 | 读取优先级 | 保存位置 |
|---------|-----------|---------|
| 文件系统用户皮肤 | 皮肤目录 `persona.md` | 直接写回皮肤目录 |
| 内置皮肤 | `$MILES_DATA_DIR/persona-overrides/{skinId}.md` > qrc 内 `persona.md` | 写到 `persona-overrides/` |

加载优先级：`persona-overrides/{skinId}.md` > 皮肤目录 `persona.md` > 空。
...
`Complete` 用于摘要压缩的同步请求。超时跟随 HTTP client（120s），取消通过 ctx 传播。摘要请求当前不走 Langfuse（Phase 2.3.3 再统一接入）。
