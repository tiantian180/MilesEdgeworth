...
- 菜单：把通用 sleep 命令转成 sleep recipe 或 returnToIdle。
...
- `miles.feedTea`：Miles 皮肤命令，转换为 `menu.tea` 动作池请求。
...
- 新增 `CustomInteractionRegistry`，给 Miles 红茶、后续检察官徽章和皮肤脚本留下高级交互入口。
- `PetRuntime` 通过 `submitActionRequest()` 执行最终播放请求。

约束：

- QML 不直接调用单击、双击、红茶、睡眠、idle、prop 的业务入口。
- `PetRuntime` 不承接红茶、单击、双击、prop 的业务判断。
...
### Phase 0.60：抽出 PlaybackController 和 RecipeRunner
...
### Phase 0.63：评估 AudioController 和 MotionController
