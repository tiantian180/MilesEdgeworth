...
- 新增 `manifest.canvas = { windowSize, imageSize, idleLoopAction }`。`windowSize` / `imageSize` 是 scale=1 的基础尺寸；Miles 当前写 `{ windowSize: 120, imageSize: 100 }`，中号 scale=2 时仍得到旧版窗口 240、动画 200。
