...
2. `SkinManifestLoader` 解析升级后的 `clickBehaviors`；保留旧 schema 兼容期内的解析路径，并在加载完成后做一次校验：每个 `zone` 必须在 `hitZones` 中存在。
3. `HitZoneMatcher::clickPoolForPoint()` 改写：只遍历 manifest `clickBehaviors.singleClick`，对每条 `{zone, pool}` 调用现有 `hitZoneContainsPoint()`，命中即返回。删除 `hitZoneIdForClickPool()` 这个写死 switch。
