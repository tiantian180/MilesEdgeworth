...
> **当前代码状态（Phase 1 收官后）**：`PetRuntime` 仍是最终播放执行入口，但鼠标、菜单、idle、Prop、agent expression 等行为判断已经经由 `PetEventBridge` / `InteractionPipeline` 转换成 `ActionRequest`。`SkinManifestLoader`、`ActionPoolSelector`、`BehaviorTriggerEngine`、`HitZoneMatcher`、`GestureTracker`、`PropController`、`AudioController`、`SkinCommandResolver`、`ExpressionMappingResolver` 与 C++ `CustomInteraction` Host API 已落地。`PlaybackController`、`RecipeRunner`、`MotionController` 仍是 Phase 0.74+ 拆分方向。
...
| action pool | 已迁到 `ActionPoolSelector` | 继续保持纯逻辑、可测试 |
