...
belly     (120, 150) -> idle_pointing 或 bow
legs      (120, 200) -> back_away 或 idle_look_down
```

有些分区在旧版就是随机候选，例如头部、小臂、肚子和腿部；运行时测试用候选集合断言，避免把随机性误写成固定动作。后续如果继续校准左朝向其它分区，可以沿用同一套坐标和候选集合模式。
