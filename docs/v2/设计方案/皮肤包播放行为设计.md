...
当前 `schemaVersion: 4` 的可运行 Miles manifest 把素材、行为和高级交互配置放在同一个 `manifest.json`。读文档时可以先按下表定位：基础字段负责”这个皮肤是谁和多大”，动画字段负责”能播什么”，行为字段负责”什么事件触发什么”，扩展字段负责”语音、Prop 和高级交互”。
