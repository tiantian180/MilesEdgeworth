...
这一步仍沿用 manifest 里的 `tea.drinkThenBow` 和 `sleep.enterLoopExit`。后续若需要更接近旧版随机喝茶素材，可把 `menu.tea` 从单一 recipe 改成候选池，例如在两组茶杯动作之间随机抽取。

## 当前实现和 Phase 0.17

Phase 0.17 补齐旧版“喂食红茶”的第二组 GIF，并新增核心手感覆盖检查。旧版菜单触发喝茶时，会按当前朝向在两组茶杯动作中随机选择：右朝向使用 `tea0` 或 `tea2`，左朝向使用 `tea1` 或 `tea3`。v2 现在用两个 action 和两个 scenario recipe 表达这个差异：
...
teaAlt.drinkThenBow:
  tea_alt -> bow -> idle_stand

menu.tea:
  weighted random [tea.drinkThenBow, teaAlt.drinkThenBow]
```
