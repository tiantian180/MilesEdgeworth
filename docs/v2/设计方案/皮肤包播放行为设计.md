...
| 使用者 | 主要诉求 | 需要维护的内容 |
| --- | --- | --- |
| 普通换肤作者 | 替换角色素材，配置 idle、点击、菜单、agent 状态动作 | assets、actions、recipes、animation pools、hit zones |
...
| AnimationPools | 候选集合，用于随机 idle、点击、LLM 表达标签等场景 |
...
| 路径 | 职责 | 是否人工维护 |
| --- | --- | --- |
| `manifest.json` | 描述 canvas、anchors、clips、actions、recipes、animationPools、props、fallback | 是（clips 由构建脚本读取生成 GIF） |
| `behavior.json` | 描述 idle 随机、点击区域、双击、右键菜单、agent 状态映射 | 是 |
| `interactions/` | 高级可信交互代码，例如检察官徽章、复杂道具、特殊音效流程 | 是 |
| `assets/` | 正本素材，按资源语义组织 | 是 |
| `generated/` | 构建脚本从 clips 定义切分生成的完整 GIF 文件 | 否（构建脚本自动生成） |

推荐按资源语义组织素材，而不是按触发来源组织素材。比如抱胸思考既可以用于随机 idle，也可以用于 agent thinking，正本素材只保留一份，触发差异通过 Recipe / AnimationPool / BehaviorRule 表达。
...
| `actions` / `recipes` / `animationPools` | 主体动画、时间线、随机候选池 | `PetRuntime` / `AnimationPoolSelector` |
...
皮肤包有两个核心文件：`skin.json`（元信息，用于扫描发现）和 `manifest.json`（行为与素材声明，用于加载后运行）。二者的职责划分见《皮肤包分发与加载机制设计》§2。

当前 `schemaVersion: 4` 的可运行 Miles manifest 把素材、行为和高级交互配置放在 `manifest.json`。读文档时可以先按下表定位：基础字段负责”这个皮肤多大”，动画字段负责”能播什么”，行为字段负责”什么事件触发什么”，扩展字段负责”语音、Prop 和高级交互”。

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `schemaVersion` | number | 是 | manifest schema 版本。当前 Miles 使用 `4`。 |
| `id` / `name` | string / string | 可选 | manifest 内可保留 id/name 用于调试，但运行时权威来源是 `skin.json`。 |
...
| `clips` | object | 有帧段切分时需要 | 从原始 GIF 切分的片段定义，构建脚本据此生成完整 GIF。 |
| `animationPools` | object | 随机/候选行为需要 | 多个候选 recipe/action 的集合，按权重随机选择。 |
...
| `motion` | object | 目标点移动需要 | 移动速度和吸附参数：`walkSpeed`（像素/秒, scale=1）、`runSpeed`（同上）、`snapDistance`（像素, scale=1）。详见《移动系统与工具调用设计》§5.4。 |
...
```json
{
  ...
  "actions": {
    "idle_stand": {
      ...
      "loopMode": "loop",
      "variants": {
        "right": { "clip": "file:assets/body/idle/stand-right.gif" },
        "left": { "clip": "file:assets/body/idle/stand-left.gif" }
      }
    }
  }
}
...
Action variant 的 `clip` 字段统一描述"播放什么"。值有两种形式：`file:assets/...` 直接引用完整 GIF 文件；裸名字（如 `"thinking.enter.right"`）引用 `clips` section 中定义的预切分片段，运行时从 `generated/clips/` 加载构建脚本生成的 GIF。运行时只播放完整 GIF（统一使用 QMovie），不做运行时帧段截取。
...
`clickBehaviors` 把 hit zone 和 animation pool 配对，让框架代码不需要内置任何皮肤命名。`InteractionPipeline` 按 `singleClick` 声明顺序把候选 zone 交给 `HitZoneMatcher`，命中后再把对应入口转成 `ActionRequest`。`doubleClick` 同样以列表方式声明，支持普通候选池和高级 Custom Interaction 入口（详见后文）。
...
| 层级 | 回答的问题 | 对应 manifest 字段 |
| --- | --- | --- |
| Assets | 文件在哪里 | `file:assets/...` 路径（在 `clip` / `asset` 字段中引用） |
| Clip | 从文件取哪一段 | `clips` section + 构建脚本生成 `generated/clips/` |
| Action | 这个动作是什么意思 | `actions` |
| Recipe | 这次请求怎样编排播放 | `recipes` / `steps` |
...
Clip 是最小可播放片段，描述”从哪个素材取哪一段”。仅用于需要从原始 GIF 切分帧段的场景；完整 GIF 的 action 不需要定义 clip，直接在 variant 的 `clip` 字段中使用 `file:` 引用即可。

构建脚本读取 `clips` 定义，将每个 clip 切分为独立的完整 GIF 文件，输出到 `generated/clips/` 目录。运行时只播放完整 GIF（统一使用 QMovie），不做运行时帧段截取。

`clips` 字段定义：

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `source` | string | 原始素材路径，使用 `file:assets/...` 格式。 |
| `frameRange` | `[start, end]` | 闭区间帧段，1-based 帧号。 |

生成规则：clip 名即文件名（convention over configuration）。`thinking.enter.right` → `generated/clips/thinking.enter.right.gif`。
...
```json
{
  "clips": {
    "thinking.enter.right": {
      "source": "file:assets/body/gestures/thinking-right.gif",
      "frameRange": [1, 4]
    },
    "thinking.loop.right": {
      "source": "file:assets/body/gestures/thinking-right.gif",
...
Action variant 引用 clip 的 `clip` 字段有两种值形式：

| 值 | 含义 | 示例 |
| --- | --- | --- |
| 裸名字 | 查 `clips` section，播放 `generated/clips/{name}.gif` | `”clip”: “thinking.enter.right”` |
| `file:` 前缀 | 直接引用皮肤包内的完整 GIF 文件 | `”clip”: “file:assets/body/idle/stand-right.gif”` |
...
| `variants` | object | 朝向或移动方向变体。变体写 `clip` 字段引用 clip 名或 `file:` 路径。 |
...
```json
{
  "actions": {
    "objecting": {
      ...
      "variants": {
        "right": { "clip": "file:assets/body/interaction/objecting-right.gif" },
        "left": { "clip": "file:assets/body/interaction/objecting-left.gif" }
      }
    }
  }
}
...
```json
{
  "actions": {
    "walk": {
      ...
      "variants": {
        "east": {
          "clip": "file:assets/body/locomotion/walk-east.gif",
          "movement": { "dx": 8.4, "dy": 0 }
        },
        "west": {
          "clip": "file:assets/body/locomotion/walk-west.gif",
          "movement": { "dx": -8.4, "dy": 0 }
        }
      }
    }
...
AnimationPool 用于“从多个候选里选择一个”。它可以被 idle、点击、双击、菜单、LLM 表达标签复用。
...
每个 entry 中 `action`、`recipe`、`pool` 三者至多出现一个（互斥）。
...
选择策略通常由调用方决定：`AnimationPoolSelector` 当前按权重选候选；`ExpressionMappingResolver` 支持 `first_available` 和 `weighted_random`。后续如果给 pool 本身增加 `selection`，需要同步更新静态校验。
...
| `entries` | array | 候选入口。 |
| `entries[].pool` | string | 可选，从指定 pool 中选择。 |
| `entries[].recipe` | string | 可选，请求指定 recipe。 |
| `entries[].action` | string | 可选，请求指定 action。 |
| `entries[].command` | string | 可选，运行时命令：`returnToIdle`、`toggleFacing`。 |
| `entries[].weight` | number | 候选权重。仅含 `weight` 时为跳过项（不执行任何动作）。 |
| `entries[].when` | object | 候选级触发条件。 |

每个 entry 中 `pool`、`recipe`、`action`、`command` 至多出现一个（互斥）。仅含 `weight` 表示跳过（no-op）。
...
| `pool` | string | 从 animation pool 中选择。 |
...
| 皮肤定制命令 | 某个皮肤提供的简单菜单命令，通常映射到 animation pool 或 recipe | Miles 的喝红茶 | Skin command |
...
- `miles.feedTea` 是皮肤定制命令，由 manifest `skinCommands` 声明，经 `SkinCommandResolver` 映射到 `menu.tea` animation pool。
...
| `request.pool` | string | 可选，从指定 pool 中选择。 |
| `request.recipe` | string | 可选，请求指定 recipe。 |
| `request.action` | string | 可选，请求指定 action。 |
| `request.command` | string | 可选，运行时命令：`returnToIdle`、`toggleFacing`。 |
...
```json
{
  "props": {
    "prosecutor_badge": {
      "asset": "file:assets/props/prosecutor_badge/prosecutor-badge.png",
      "width": 94,
      "height": 94,
      "visualWidth": 24,
      "visualHeight": 24,
...
`动画素材盘点.md` 记录的是旧 GIF 事实；本节说明这些事实如何沉淀成 manifest。Schema v4 中，帧段定义统一写在顶层 `clips` section，action variant 通过 `clip` 字段引用 clip 名。构建脚本根据 `clips` 定义预切分 GIF 到 `generated/clips/`，运行时只播放完整 GIF。
  ...
  "actions": {
    "thinking": {
      ...
      "phases": {
        "enter": {
          "loopMode": "once",
          "variants": { "right": { "clip": "thinking.enter.right" } }
        },
        "loop": {
          "loopMode": "loop",
          "variants": { "right": { "clip": "thinking.loop.right" } }
        },
        "exit": {
          "loopMode": "onceThenHold",
          "variants": { "right": { "clip": "thinking.exit.right" } }
        }
      }
    }
  },
...
{
  "clips": {
    "back_away.enter.right": { "source": "file:assets/body/interaction/back-right.gif", "frameRange": [1, 7] },
    "back_away.hold.right": { "source": "file:assets/body/interaction/back-right.gif", "frameRange": [7, 7] },
    "back_away.exit.right": { "source": "file:assets/body/interaction/back-right.gif", "frameRange": [8, 9] }
  },
  "actions": {
    "back_away": {
      "label": "后撤警惕",
      ...
      "phases": {
        "enter": {
          "loopMode": "once",
          "variants": { "right": { "clip": "back_away.enter.right" } }
        },
        "hold": {
          "loopMode": "hold",
          "variants": { "right": { "clip": "back_away.hold.right" } }
        },
        "exit": {
          "loopMode": "onceThenHold",
          "variants": { "right": { "clip": "back_away.exit.right" } }
        }
      }
    }
  }
...
ActionRequest 是运行时播放请求的统一形态。请求对象通过存在哪个键来确定类型，不再使用 `type` 字段。
...
| `pool` | string | 可选，从某个 animation pool 中选择。 |
| `command` | string | 可选，运行时命令：`returnToIdle`（回到 idle，有特殊语义：已 idle 时为 no-op、清空活跃 recipe、先播 exit phase）、`toggleFacing`。 |

`action`、`recipe`、`pool`、`command` 四者至多出现一个（互斥）。loader 或 schema 校验应拒绝同时出现多个 dispatch key 的请求对象，以保证行为确定。
