...
`clickBehaviors` 把 hit zone 和动作池配对，让框架代码不需要内置任何皮肤命名。`HitZoneMatcher` 仅按声明顺序遍历 `singleClick` 列表选第一个命中的入口。`doubleClick` 同样以列表方式声明，支持普通候选池和高级 Custom Interaction 入口（详见后文）。
...
sequenceDiagram
    ...
    loop 注册顺序遍历
        ...
        CI->>Runtime: hostApi.emitRecipe / spawnProp / playSound ...
...
- handler 的输出请求会和默认逻辑的请求按 `ActionRequest.priority` 与 `interruptHint` 合并，由 `PetRuntime` 按现有规则处理；CI 不与 PetRuntime 直接交互。
...
public:
    ...
    void emitRecipe(const QString &recipeId, const RecipeParams &params = {});
    ...
    QVariantMap manifestConfig(const QString &handlerId) const;  // 读 manifest 的 customInteractionConfig[id]
...
{
    const auto config = host.manifestConfig(id());
    ...
    case PetEventType::PointerDoubleClick: {
        ...
        host.spawnProp(config.value("badgePropId").toString());
        host.skipDefault();
    ...
    case PetEventType::PropClicked:
        if (event.propId == config.value("badgePropId").toString()) {
            host.emitRecipe(config.value("bowRecipe").toString());
            host.skipDefault();
        }
        return {};
    case PetEventType::PropExpired:
        if (event.propId == config.value("badgePropId").toString()) {
            host.emitRecipe(config.value("pickupRecipe").toString());
            host.skipDefault();
        }
        return {};
    default:
