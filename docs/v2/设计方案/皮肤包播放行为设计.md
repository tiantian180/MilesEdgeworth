...
后续 Phase 0.41 已把 `belly / legs` 拆成更细的确定性子区域。长期设计以“按优先级检测多个 hit zone”为准；区域数量由皮肤 manifest 决定。
...
- `singleClickTimer`：单击延迟确认，使用旧版 300ms 双击判定窗口，避免双击时先触发一次单击分区反应。
...
- `clampedPetWindowPosition(position)`：根据旧版角色身体点位夹住窗口坐标。
...
Phase 0.46 开始，当前夹取策略已经使用旧版身体点位做边界判断，而不是按完整透明窗口外壳做保守夹取。旧版用 `(36,10)`、`(63,10)`、`(36,90)`、`(63,90)` 这四个点近似 Miles 的可见身体范围，因此透明留白可以略微越过屏幕边缘，角色主体仍留在屏幕内。

当前实现仍把这组常量写在 `DesktopShellController` 中，并通过 `PetRuntime.petScale()` 同步尺寸档位。后续通用换肤时，应把这些常量抽成皮肤 manifest 的 `bodyBounds` 或 `movementBounds`：
...
Phase 0.47 继续恢复旧版“双屏选项”。在 `DesktopShellController.screenLayoutMode` 为 `primaryLeft` 或 `primaryRight` 时，X 轴边界使用所有屏幕 `geometry()` 合成的虚拟桌面范围，Y 轴仍使用当前屏幕高度：

```mermaid
flowchart LR
    MODE["screenLayoutMode<br/>single / primaryLeft / primaryRight"]
    SCREENS["QGuiApplication::screens()"]
    VIRTUAL["virtualDesktopGeometry()"]
    TARGET["targetScreen.geometry()"]
    CLAMP["clampedPetWindowPosition()"]

    MODE --> CLAMP
    SCREENS --> VIRTUAL
    VIRTUAL -->|"双屏模式提供 X 范围"| CLAMP
    TARGET -->|"提供 Y 范围和单屏范围"| CLAMP
```

这个阶段仍然是旧版手感迁移，不是最终路线规划。长期移动控制器应该把屏幕排列、目标点、障碍和动画方向选择统一起来；当前实现只保证旧版菜单入口和“左右屏幕可以横向跨过去”的基础行为。
...
Phase 0.38 将这两个池子的权重校准到旧版每轮移动动画结束时的真实概率。旧版先有 50% 概率不进入 `autoChangeGif()`，等价于继续当前移动动画；剩下 50% 才进入 walk/run 各自的续行分支。

```text
walk.finished，总权重 800
继续当前方向走路  400  -> 50%
沿当前方向切跑步   80  -> 10%
随机走路方向      120  -> 15%，8 个方向各 15
回到站立          200  -> 25%

run.finished，总权重 800
继续当前方向跑步  400  -> 50%
沿当前方向切走路   80  -> 10%
随机跑步方向       80  -> 10%，8 个方向各 10
回到站立          240  -> 30%
```
...
Phase 0.40 收敛“测试喝茶”入口。右键菜单里既有正式的“喂食红茶”，也有开发期使用的“测试喝茶”。这两个入口都必须进入同一个 `requestTea()`，再由 `menu.tea` 随机选择 `tea.once` 或 `teaAlt.once`。
...
- 睡眠相关状态中不会触发喝茶。
- 两组旧版茶杯 GIF 都保留随机候选。
- `tea.drinkThenBow` 这类早期实验 recipe 不再出现在运行时入口里。
...
Phase 0.30 把检察官徽章 Prop 纳入 `PetRuntimeSmoke` 的真实行为验证。Phase 0.14 已经接入了 manifest、资源、QML 独立 Prop 窗口和运行时入口；这一阶段补上延迟生成、点击分支和自然消失分支的运行时证明。

当前 smoke 覆盖：

```mermaid
flowchart LR
    TAKE["doubleClick.takeThat"]
    DELAY["等待 700ms 延迟"]
    BADGE["currentPropId = prosecutor_badge"]
    CLICK["handlePropClicked"]
    BOW["bow.once"]
    EXPIRE["handlePropExpired"]
    PICKUP["pickup.once"]
    IDLE["idle_stand"]

    TAKE --> DELAY --> BADGE
    BADGE --> CLICK --> BOW --> IDLE
    BADGE --> EXPIRE --> PICKUP --> IDLE
```

这条验证确保 `prop.clickedRecipe` 和 `prop.expiredRecipe` 都能从运行时走到真实动作。后续如果 PropRuntime 从当前的最小纵切升级为统一 `PropCommand` / `SideEffect` 时间线，仍然要保持：`Take that` 延迟飞出徽章、点击徽章鞠躬、徽章自然消失后捡徽章。

## 当前实现和 Phase 0.31

Phase 0.31 补强睡眠循环中的鼠标交互验证。旧版睡着时，普通单击不会触发分区反应，也不会打断睡眠；双击则等同于唤醒。v2 在 PetRuntime 层保留这个边界，避免睡眠态被普通点击池或双击语音池误接管。
...
flowchart LR
    ...
    STILL["仍停在 sleep.loop"]
    DOUBLE["handleDoubleClick"]
    EXIT["sleep.exit"]
    IDLE["idle_stand"]
    MENU_WAKE["toggleSleep"]

    ENTER --> LOOP
    LOOP --> CLICK --> STILL
    LOOP --> DOUBLE --> EXIT --> IDLE
    LOOP --> MENU_WAKE --> EXIT
```

这样睡眠唤醒有两条被验证的入口：右键菜单里的“睡觉 / 唤醒”，以及旧版保留的双击唤醒。后续加入 Agent 状态或自定义交互时，睡眠态仍应作为一个较高优先级的状态边界处理。

## 当前实现和 Phase 0.32

Phase 0.32 补强单击分区的运行时验证。此前 smoke 已经覆盖脸部和 polygon 边界；这一阶段继续覆盖头部、大臂、小臂、胸口、肚子和腿部，确认这些区域都能从 `handlePrimaryClick` 进入对应 action pool。

当前 smoke 使用稳定坐标验证右朝向分区：

```text
head          (100, 40)  -> idle_tapping_head 或 idle_look_up
upperArm      (60, 85)   -> turn_around
forearm       (60, 130)  -> idle_check_watch 或 idle_shrug
chest         (120, 85)  -> idle_thinking_once
belly upper   (120, 138) -> bow
belly lower   (120, 158) -> idle_pointing
right leg L   (95, 200)  -> back_away
right leg R   (145, 200) -> idle_look_down
left leg R    (145, 200) -> back_away
left leg L    (95, 200)  -> idle_look_down
```

有些分区在旧版就是随机候选，例如头部和小臂；运行时测试用候选集合断言，避免把随机性误写成固定动作。肚子和腿部已经按旧版坐标分支拆开，避免把确定性反应误写成粗区域随机池。

## 当前实现和 Phase 0.41

Phase 0.41 把肚子和腿部从粗区域随机池拆成确定性子区域，贴近旧版 `singleClickEvent()` 的坐标判断。

```mermaid
flowchart TB
    CLICK["handlePrimaryClick"]
    BELLY_TOP["belly_bow"]
    BELLY_BOTTOM["belly_pointing"]
    LEG_BACK_R["right: left leg area"]
    LEG_DOWN_R["right: right leg area"]
    LEG_BACK_L["left: right leg area"]
    LEG_DOWN_L["left: left leg area"]

    CLICK --> BELLY_TOP --> BOW["bow.once"]
    CLICK --> BELLY_BOTTOM --> POINT["click.bellyPointing"]
    CLICK --> LEG_BACK_R --> BACK["click.legsBack"]
    CLICK --> LEG_DOWN_R --> DOWN["click.legsLookDown"]
    CLICK --> LEG_BACK_L --> BACK
    CLICK --> LEG_DOWN_L --> DOWN
```

当前 `clickBehaviors.singleClick` 的尾部变为：
...
- 头部、小臂仍是随机候选池。
- 肚子上半确定触发鞠躬，下半确定触发指点。
- 腿部根据当前朝向拆左右半边：靠角色后侧触发后退，另一侧触发低头看。

## 当前实现和 Phase 0.42

Phase 0.42 接入旧版“调整大小”菜单。旧版尺寸常量分别是 `MINI = 1`、`SMALL = 1.5`、`MEDIAN = 2`、`BIG = 3`；v2 继续使用这四档 scale，让窗口尺寸、主体动画尺寸和自动移动步长一起变化。

```mermaid
flowchart LR
    MENU["右键菜单：调整大小"]
    SIZE["petSizeId<br/>mini / small / medium / big"]
    SCALE["petScale<br/>1 / 1.5 / 2 / 3"]
    WINDOW["petWindowSize = 120 * scale"]
    IMAGE["petImageSize = 100 * scale"]
    MOVE["movementScaleFactor = scale / 2"]
...
当前默认尺寸是 `medium`，对应旧版 `MEDIAN = 2`：

```text
medium: petWindowSize = 240, petImageSize = 200
mini:   petWindowSize = 120, petImageSize = 100
small:  petWindowSize = 180, petImageSize = 150
big:    petWindowSize = 360, petImageSize = 300
```

manifest 中的 `movement.dx / movement.dy` 仍按旧版默认中号尺寸记录。运行时消费每帧移动量时会乘上 `petScale / 2`：

```text
walk.east medium dx = 8.4
walk.east mini   dx = 4.2
walk.east big    dx = 12.6
```

这样用户缩小桌宠时，走路和跑步不会显得移动过猛；放大桌宠时，移动步长也会同步变大，保留旧版按角色尺寸缩放的手感。

## 当前实现和 Phase 0.43

Phase 0.43 把检察官徽章 Prop 纳入尺寸缩放链路。旧版 `ProsecutorBadge` 的起点偏移按角色 scale 计算，飞行距离按 `600 + 150 * scale` 计算；v2 现在把这些规则显式写进皮肤 manifest，避免窗口尺寸变了但徽章仍按固定像素飞出。

```mermaid
flowchart LR
    SIZE["petScale<br/>1 / 1.5 / 2 / 3"]
    START["startOffsets<br/>按 medium 记录"]
    START_SCALE["start = startOffset * scale / 2"]
    TRAVEL["travelBase + travelPerScale * scale"]
    VISUAL["visualWidth / visualHeight<br/>按 medium 记录"]
    VISUAL_SCALE["visual = visual * scale / 2"]
    PROP["Prosecutor Badge Window"]

    SIZE --> START_SCALE
    START --> START_SCALE --> PROP
    SIZE --> TRAVEL --> PROP
    SIZE --> VISUAL_SCALE
    VISUAL --> VISUAL_SCALE --> PROP
```

当前 `prosecutor_badge` 的关键字段：
...
```text
big / scale=3:
  right start = (172, 32) * 3 / 2 = (258, 48)
  right travel = 600 + 150 * 3 = 1050
  right endX = 258 + 1050 = 1308
  visualWidth = 70 * 3 / 2 = 105

mini / scale=1:
  left start = (2, 32) * 1 / 2 = (1, 16)
  left travel = -(600 + 150 * 1) = -750
  left endX = 1 - 750 = -749
  visualWidth = 70 * 1 / 2 = 35
```

`travel` 仍保留为中号默认值，给简单 Prop 使用；当 `travelBase / travelPerScale` 存在时，运行时优先使用旧版公式。这样普通换肤可以只写固定飞行偏移，Miles 官方皮肤可以保留更细的旧版飞行手感。

## 当前实现和 Phase 0.45

Phase 0.45 恢复旧版启动入场位置。旧版窗口并不是从屏幕中央或系统默认位置出现，而是先落在屏幕左下角略微越界的位置，然后播放 `briefcaseIn` / `briefcaseStop`，形成“从屏幕外走进来”的第一印象。

旧版定位公式：

```text
startupX = screen.availableX
         - 45 * scale

startupY = screen.availableY
...
```text
main.cpp
  -> attachPetWindowToShellController()
  -> DesktopShellController.placePetWindowForStartup(PetRuntime.petScale)
  -> QWindow::setPosition(legacyStartupPosition)
```

这条路径只用于启动入场，刻意绕过普通移动边界夹取。普通 walk / run / drag 仍然走 `movePetWindowTo()` 和 `clampedPetWindowPosition()`，避免日常自动移动把桌宠带出屏幕。后续如果皮肤包需要完全自定义入场点，可以把 `legacyStartupPosition` 抽成 manifest 里的 `startupPlacement`：

```json
{
  "startupPlacement": {
    "screenAnchor": "bottomLeft",
    "offsetByScale": { "x": -45, "y": -90 },
    "bypassMovementClamp": true
  }
}
```
...
Phase 0.33 补强双击语音动作的运行时验证。`doubleClick.random` 负责随机抽取；smoke 直接播放四个旧版 recipe，确认每个 recipe 都能落到预期动作和默认语音：

```text
doubleClick.holdIt    -> crossed   + holdit0.wav
doubleClick.takeThat  -> objecting + takethat0.wav
doubleClick.objection -> objecting + objection0.wav
doubleClick.eureka    -> objecting + eureka0.wav
```

语言覆盖仍由 Phase 0.24 的 `doubleClick.random.zh` 负责：中文候选池排除缺少中文语音资源的 Eureka 分支；具体 recipe 仍可按 `sounds` 选择语言资源。

## 当前实现和 Phase 0.34

Phase 0.34 补强 8 方向走路 / 跑步的运行时验证。Phase 0.10 已经守住 manifest、资源 alias、recipe 和 QML 移动入口；这一阶段让 `PetRuntimeSmoke` 真正播放全部移动 recipe，验证每个方向的移动符号、朝向更新和速度关系。

当前 smoke 覆盖：

```text
east      dx > 0, dy = 0, facing = right
west      dx < 0, dy = 0, facing = left
northEast dx > 0, dy < 0, facing = right
northWest dx < 0, dy < 0, facing = left
southEast dx > 0, dy > 0, facing = right
southWest dx < 0, dy > 0, facing = left
north     dx = 0, dy < 0, 保留原朝向
south     dx = 0, dy > 0, 保留原朝向
```

同一方向下还会比较 `run` 和 `walk` 的移动速度，确保跑步不是只换了动画资源。后续进入路线控制器后，可以把“移动方向 -> 动画方向”的选择逻辑升级为路径驱动，但这张 8 方向运行时表仍然是还原旧版自由游走的基础验收线。

## 当前实现和 Phase 0.35

Phase 0.35 补强随机 idle 非移动候选的运行时验证。`idle.random` 中的 walk / run 候选已经由 Phase 0.34 覆盖；这一阶段逐个播放站立小动作和转身候选，确认 recipe 能进入预期 action。
...
这条 smoke 不替代长期手感试玩；它先保证随机待机池里的每个非移动候选都能被运行时播放。后续还需要结合长时间视觉试玩，校准权重、间隔和动作结束后的续行动作。

## 当前实现和 Phase 0.36

Phase 0.36 将 `idle.random` 的候选和权重校准到旧版 `STAND` 自动切换结构。旧版站立循环结束后，会按下面的概率选择下一段动作：
...
v2 manifest 使用整数权重表达同一比例，当前总权重为 400：

```text
idle.flipStand     32
turn.once          48
10 个 once 小动作  240，每个 24
8 个 walk 方向     40，每个 5
8 个 run 方向      40，每个 5
```

`bow.once` 和 `objecting.once` 仍然保留为可复用 recipe，但它们不进入 `idle.random`。鞠躬来自单击肚子、徽章点击后的反馈等场景；异议来自双击语音动作。这样随机待机池只承担旧版站立自动切换职责，特殊交互继续通过各自的 action pool 或 recipe 触发。

## 当前实现和 Phase 0.37

Phase 0.37 将随机待机的触发时机从固定 7 秒计时器改为“站立循环完成事件”。旧版逻辑绑定在 `QMovie::frameChanged`：当 `STAND` GIF 播到最后一帧时，运行时按 0.7 概率进入 `autoChangeGif()`，按 0.3 概率继续播放站立循环。

v2 的对应链路如下：

```mermaid
flowchart LR
    A["AnimatedImage 播放 idle_stand"] --> B["currentFrame 到最后一帧"]
    B --> C["PetRuntime.handleIdleLoopFinished()"]
    C --> D["behaviorTriggers.idle.loopFinished"]
    D --> E{"按 manifest 权重抽取"}
    E -->|"pool: idle.random"| F["playActionFromPool('idle.random')"]
    E -->|"type: none"| G["继续 idle_stand 循环"]
```

`PetRuntime` 仍然会检查当前状态：只有 `currentState == idle`、没有正在播放 recipe、当前 action 是 `idle_stand` 时，站立循环事件才会生效。这样喝茶、双击语音、拖拽晃动、睡眠和徽章反馈不会被待机随机动作打断。

## 当前实现和 Phase 0.52

Phase 0.52 将 `idle.loopFinished` 的概率从 C++ 迁移到皮肤 manifest。运行时只保留通用的 `behaviorTriggers` 执行器，具体“触发条件、候选目标、权重比例”由皮肤决定：
...
这个结构的重点是把“旧版 Miles 的 70/30 节奏”放回皮肤层，而不是写死到 `PetRuntime`。其它皮肤可以把同一个事件配置成更高频的小动作、完全不随机待机，或者进入自己的候选池。

```mermaid
flowchart TB
    EVENT["运行时事件<br/>idle.loopFinished"]
    WHEN["when 条件<br/>state/action/active recipe"]
    ENTRIES["entries 权重池"]
    NONE["type: none<br/>保持当前 stand"]
    POOL["type: pool<br/>进入 idle.random"]

    EVENT --> WHEN
    WHEN -->|"匹配"| ENTRIES
    WHEN -->|"不匹配"| IGNORE["忽略事件"]
    ENTRIES --> NONE
    ENTRIES --> POOL
```
