...
sequenceDiagram
    ...
    participant Registry as SkinRegistry
    participant Loader as SkinManifestLoader
    participant Menu as PetContextMenu
    participant Runtime as PetRuntime

    App->>Registry: discoverAll()
    Registry->>Loader: read skin.json from user / portable / qrc
    Loader-->>Registry: SkinDescriptor list
    Registry-->>Menu: available skins
    Menu->>Runtime: setActiveSkin(skinId)
    Runtime->>Loader: load descriptor rootUrl + manifest.json
    Loader->>Loader: resolve file: URLs under root
...
```
内置 Miles:    file:assets/body/foo.gif  →  qrc:/skins/miles-edgeworth/assets/body/foo.gif
用户皮肤:      file:assets/body/foo.gif  →  file:///.../skins/my-skin/assets/body/foo.gif
```

兼容旧 manifest：
- 已存在的 `qrc:/...` 绝对 URL 仍正常工作（内置 Miles 过渡期兼容）
- Studio 导出的 manifest 始终用 `file:` scheme
...
| 皮肤类型 | 生成时机 | 说明 |
| --- | --- | --- |
| 内置皮肤（qrc） | 编译前，作为构建步骤 | 切分脚本输出到 `resources/skins/.../generated/clips/`，再由 qrc 纳入 |
...
`SkinManifest` 加一个 `rootUrl` 字段保存皮肤根路径，loader 在加载后把 `file:foo.gif` 等所有资源 URL 一次性替换为绝对 URL（`qrc:/.../foo.gif` 或 `file:///.../foo.gif`）。

落地前后的差异：

| 能力 | 改造前 | 当前代码 |
| --- | --- | --- |
| 皮肤发现 | 无扫描；固定内置 Miles | `discoverAll()` 扫用户目录、便携目录、内置 qrc |
| manifest 加载 | `loadFromResource(":/pet/manifest.json")` | `loadFromDirectory()` / qrc descriptor 二选一 |
| 资源 URL | manifest 直接写 `qrc:/pet/...` | manifest 写 `file:assets/...`，loader 解析成绝对 URL |
| 皮肤切换 | 无 active skin 切换 | Runtime 停止当前播放，加载新 manifest，重建菜单 |
| Studio 协作 | 无 | Studio 编辑用户目录皮肤，桌宠重载当前皮肤 |
...
文件系统皮肤的 asset URL 解析时强制限制在皮肤根目录下，防止 `file:../../../../etc/passwd` 这类穿越。loader 实现时用 `QDir::cleanPath` + `startsWith(rootUrl)` 校验。
...
3. **内置 Miles 改用 `file:` scheme**：与文件系统皮肤包写法统一
4. **应用菜单皮肤切换 + 重载入口**
...
| Miles 内置 manifest | URL 从 `qrc:/...` 改为 `file:...`（兼容期内两者并存） |
