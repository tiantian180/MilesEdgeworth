...
> **当前代码状态（Phase 1 收官后）**：桌宠本体已经迁到原生 `PetSurfaceWindow`（QWidget + QMovie），旧版手感主链路已经通过 `PetEventBridge -> InteractionPipeline -> ActionRequest -> PetRuntime` 收敛。`CustomInteraction`、`AudioController`、`PropController`、`ExpressionMappingResolver`、文件系统皮肤加载、皮肤切换和皮肤热重载已落地；Go sidecar、ChatWindow、DashboardWindow 和 Pet Skin Studio 仍是后续规划。
...
MilesEdgeworth/
  apps/
    desktop/
      ...
      qml/
        ...
        ChatWindow.qml       # Phase 2+ 待实现
        DashboardWindow.qml  # Phase 2+ 待实现
        components/
      resources/

    agent-core/              # Phase 2+ 待实现
      go.mod
      cmd/miles-agent/
      internal/
        api/
  ...
  assets/                    # 长期外置皮肤规划；当前 Miles 内置在 apps/desktop/resources/skins/
    skins/
      miles-edgeworth/
        skin.json
        manifest.json
...
当前内置 Miles 皮肤位于 `apps/desktop/resources/skins/miles-edgeworth/`，通过 `qrc:/skins/miles-edgeworth/` 暴露为内置皮肤包。运行时也会扫描用户皮肤目录和应用同级便携皮肤目录；manifest 里的资源统一使用 `skin:assets/...`，由 `SkinManifestLoader` 在加载时解析为 `qrc:/skins/...` 或 `file://...` 绝对 URL。文件系统皮肤加载设计见 `皮肤包分发与加载机制设计.md`。
...
| **皮肤包内容** | HitZone、Action、Recipe、ActionPool、BehaviorRule、CustomInteraction、Prop 资源、capability 声明本身 | `manifest.json` + 皮肤资源 | 独立开发者工具（编辑 manifest），或纯手写 JSON | 修改后皮肤需重新加载（桌宠应用支持"重载当前皮肤"操作） |
...
落地的具体编辑器在后续设计文档逐个铺开，例如 `HitZone 交互区域系统设计.md` 描述了 HitZone 编辑 panel；它属于 **Pet Skin Studio**（`apps/pet-skin-studio/`）这个统一开发者工具的一部分。Pet Skin Studio 的整体框架（皮肤选择、panel 切换、状态栏等）由独立设计文档 `Pet Skin Studio 工具设计.md` 描述（待补）。未来的 Recipe / ActionPool / Asset 浏览等编辑功能都以 panel 形式进入同一个 Studio 应用，不再分散成多个工具。

### 9.y 皮肤包的分发与加载

为了让最终用户**无需 C++/Qt 编译环境就能换皮肤**，皮肤包加载已经从单一 Qt resource（`qrc:`）扩展为同时支持文件系统皮肤：

- 应用扫描三个路径：用户数据目录、便携同级目录、内置 qrc
- manifest 内部使用 `skin:` URL scheme，loader 在加载时解析为绝对 URL，让同一份 manifest 在两种位置都能工作
...
Phase 0.65 – 0.73 已完成并进入阶段记录沉淀：

| 阶段 | 当前代码结果 |
| --- | --- |
| 0.65 | 架构门禁脚本与过时断言清理完成。 |
| 0.66 | 单击 / 双击行为迁入 `manifest.clickBehaviors`，`HitZoneMatcher` 只返回 zone。 |
| 0.67 | `GestureTracker` 接管拖拽晃动识别。 |
| 0.68 | rest、startup、action.completed、movement facing map 迁入 manifest。 |
| 0.69 | canvas、尺寸菜单、idle loop 来源和原生 surface 收尾。 |
| 0.70 | C++ `CustomInteraction` / Host API / Registry 分发落地。 |
| 0.71 | 检察官徽章作为 Miles 皮肤侧 CI 回归。 |
| 0.72 | 语音语言作为 `manifest.audio.voiceLanguages` + `AudioController` 可选能力。 |
| 0.73 | `expressions` / `expressionMappings` 和 `agent.expressionRequested` 链路落地。 |

`behavior.json` 拆分在 Phase 1 不做。所有行为字段继续写在 `manifest.json`，loader 一处解析；当出现多皮肤共享行为或多档位行为需求时再迁移。

PlaybackController / RecipeRunner / MotionController 等内部 controller 的进一步拆分不阻塞 Phase 2，可与 AI 接入并行推进。`AudioController` 与当前版 `GestureTracker` 已落地。
