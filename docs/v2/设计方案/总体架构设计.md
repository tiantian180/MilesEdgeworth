...
当前内置 Miles 皮肤位于 `apps/desktop/resources/skins/miles-edgeworth/`，通过 `qrc:/skins/miles-edgeworth/` 暴露为内置皮肤包。运行时也会扫描用户皮肤目录和应用同级便携皮肤目录；manifest 里的资源统一使用 `file:assets/...`，由 `SkinManifestLoader` 在加载时解析为 `qrc:/skins/...` 或 `file://...` 绝对 URL。文件系统皮肤加载设计见 `皮肤包分发与加载机制设计.md`。
...
- manifest 内部使用 `file:assets/...` 前缀，loader 在加载时解析为绝对 URL，让同一份 manifest 在两种位置都能工作
