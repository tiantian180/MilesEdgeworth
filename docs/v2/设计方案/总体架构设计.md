...
为了让最终用户**无需 C++/Qt 编译环境就能换皮肤**，皮肤包从 Qt resource（`qrc:`）改为支持文件系统加载：
...
详见 `皮肤包分发与加载机制设计.md`。这是 Pet Skin Studio 落地的前置条件——Studio 编辑的是文件系统上的皮肤目录。
...
1. **架构对齐与门禁（0.65）**：在动 schema / 代码前，先落地静态门禁脚本，把 `阶段记录/v1 手感回归与定制化接入.md` 第 2 节里的硬编码点转写成 CTest 可执行断言；同步清理 Phase 0.64 之后已经失效的静态断言；把长期 doc 中混入的时态内容收敛到阶段记录。已收口的断言迁入对应 Phase 检查，门禁脚本保持绿色。
