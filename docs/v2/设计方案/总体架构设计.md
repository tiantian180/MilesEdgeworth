...
> **当前代码状态（Phase 2.0 合入后）**：桌宠本体已经迁到原生 `PetSurfaceWindow`（QWidget + QMovie），旧版手感主链路已经通过 `PetEventBridge -> InteractionPipeline -> ActionRequest -> PetRuntime` 收敛。`CustomInteraction`、`AudioController`、`PropController`、`ExpressionMappingResolver`、文件系统皮肤加载、皮肤切换和皮肤热重载已落地。Go sidecar（`apps/agent-core`）、QML `ChatWindow` 和 C++ `ChatController` 已在 Phase 2.0 落地，mock provider 验证了完整 HTTP + SSE 链路；真实 provider、用户配置、会话历史等能力在后续 Phase 2.x 逐步实现。DashboardWindow 和 Pet Skin Studio 仍是后续规划。
...
- 管理模型配置文件（`providers.json`），启动 sidecar 时通过环境变量注入 API key 和模型参数。详见 `模型配置与密钥存储设计.md`。
...
> Provider 配置由 Qt 侧 `providers.json` 管理，通过重启 sidecar 注入。如果未来需要 Dashboard 热更新，可增加 `GET/PUT /v1/config`。
...
- JSON 配置文件（Qt 侧）：`providers.json` 存储 API key 和模型配置，文件权限 0600。详见 `模型配置与密钥存储设计.md`。

Dashboard 修改配置时：

```text
Dashboard / 设置窗口
=> Qt 侧写入 providers.json
